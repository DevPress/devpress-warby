...
add_action( 'wp', 'warby_setup_author' );
 ...
 */
function warby_footer_class() {

	$count = warby_count_widgets( 'footer' );
 ...
 */
function warby_count_widgets( $sidebar_id ) {
 ...
 */
function warby_embed_container( $html, $url, $attr ) {
	...
	if ( isset( $attr['width'] ) ) {
		wp_enqueue_script( 'warby-fitvids' );
	}

	return $html;
}
add_filter( 'embed_handler_html', 'warby_embed_container', 10, 3 );
add_filter( 'embed_oembed_html' , 'warby_embed_container', 10, 3 );
 ...
 */
function warby_update_check() {

	$ver = get_theme_mod( 'version', false );

	// Return if update has already been run
	if ( version_compare( $ver, WARBY_VERSION ) >= 0 ) {
		return;
	}
	...
	set_theme_mod( 'version', WARBY_VERSION );

}
add_action( 'after_setup_theme', 'warby_update_check' );
