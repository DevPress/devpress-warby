...
function warby_jetpack_setup() {
	 ...
	 */
	add_image_size( 'warby-testimonial', 90, 90, true );
	...
	add_theme_support( 'infinite-scroll', array(
		'container' => 'main',
		'render'    => 'warby_infinite_scroll_render',
		'footer'    => 'colophon'
	) );

}
add_action( 'after_setup_theme', 'warby_jetpack_setup' );

/**
 * Flush the Rewrite Rules for the testimonials CPT after the user has activated the theme.
 */
function warby_flush_rewrite_rules() {
	flush_rewrite_rules();
}
add_action( 'after_switch_theme', 'warby_flush_rewrite_rules' );
 ...
 */
function warby_infinite_scroll_render() {
	while ( have_posts() ) {
		the_post();
 ...
 */
function warby_infinite_scroll_has_footer_widgets( $has_widgets ) {
...
add_filter( 'infinite_scroll_has_footer_widgets', 'warby_infinite_scroll_has_footer_widgets' );
